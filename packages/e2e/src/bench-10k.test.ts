...
test('Benchmark Writer with 10,000 char string', async ({ page }) => {
  ...
  // Generate 10,000 char string
  const text10k = generateRandomString(10000);

  // Type 10,000 char string into the editor
  await editorHandle.type(text10k);
  ...
  // Benchmark it
  bench.add('Generate QrCode Gif from 10,000 char string', async () => {
  ...
  saveBenchMark('bench-10k.json', bench.table());
});
